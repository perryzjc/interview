...
[[相似性搜索|返回 相似性搜索]] | [[向量数据库|相关: 向量数据库]]

## 概述

近似最近邻搜索 (Approximate Nearest Neighbor, ANN) 是一种在高维[[向量 (Vector)|向量]]空间中**快速查找**与查询向量**近似**相似（距离最近）的邻居的算法。
...
在处理[[向量数据库|向量数据库]]中数百万甚至数十亿级别的海量高维向量时，精确 KNN 搜索的计算成本过高，无法满足实时查询的需求，因此 ANN 成为了**实际应用中的标准方法**。
...
*   **评估技术方案**: 了解不同的 ANN 算法有不同的特点和适用场景，可以参与关于[[向量数据库|向量数据库]]或 ANN 算法选型的讨论。
*   **设定合理预期**: 向[[沟通协作/利益相关者管理|利益相关者]]解释为什么搜索结果是“近似”相关的，而不是绝对完美的。

## 总结

ANN 是解决大规模高维向量[[相似性搜索|相似性搜索]]效率问题的关键技术。它通过牺牲少量精度换取速度和效率，是现代[[向量数据库|向量数据库]]和许多 AI 应用（如 [[检索增强生成 (RAG)|RAG]]）的核心引擎。产品经理理解 ANN 的基本原理和其速度-精度的权衡，有助于设定合理的产品预期和评估相关技术方案。
